...
test('getComponents', async (t) => {
    ...
    t.deepEqual(
        components.filter(
            (c) => typeof c.room !== 'string' || c.room.length === 0,
        ),
        [],
        'all components have a room',
    )
    t.deepEqual(
        components.filter(
            (c) => typeof c.type !== 'string' || c.type.length === 0,
        ),
        [],
    ...
    t.deepEqual(
        components.filter(
            (c) => typeof c.uuid !== 'string' || c.uuid.length !== 36,
        ),
        [],
        'all components have a uuid',
    )
})
