...
export class ElightsDynamicPlatform implements DynamicPlatformPlugin {
  ...
  private readonly elightsComponents = new Map<string, ElightsRelayAccessory>();
    ...
    for(const c of components) {
      if(c.type === 'RelayOutput') {
          ...
          if(existingAccessory) {
            ...
            this.elightsComponents.set(c.uuid, new ElightsRelayAccessory(this, existingAccessory))
          }
          else {
            const accessory = new this.api.platformAccessory(`${c.room}/${c.name}`, c.uuid);
            this.elightsComponents.set(c.uuid, new ElightsRelayAccessory(this, accessory))
